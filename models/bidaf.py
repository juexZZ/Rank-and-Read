...
class BidafModel(nn.Module):
    ...
    def forward(self, passage, p_lengths, question, q_lengths):
        ...
        # [b, p_num_tokens, 5*h] -> [b, p_num_tokens, 1] -> [b, p_num_tokens]
        start_projection = self.start_projection(start_input).squeeze(2)
        # Mask
        start_logits = start_projection*p_mask + (p_mask-1)*1e20 # those masked position has VERY NEGATIVE value
        ...
        # Applies the final log-softmax to get the actual log-probability, because the loss function use log likelihood
        # vectors.
    ...
    def get_best_span(cls, start_log_probs, end_log_probs):
        ...
        if isinstance(start_log_probs, Variable):
            start_log_probs = start_log_probs.data.cpu()
        if isinstance(end_log_probs, Variable):
            end_log_probs = end_log_probs.data.cpu()

        batch_size, num_tokens = start_log_probs.size()
        start_end = torch.zeros(batch_size, 2).long()# start,end
        max_val = start_log_probs[:, 0] + end_log_probs[:, 0]# log sum is actually prob product
        max_start = start_log_probs[:, 0]
        arg_max_start = torch.zeros(batch_size).long()
        # find max prob and position
        for batch in range(batch_size):
            _start_lp = start_log_probs[batch]
            _end_lp = end_log_probs[batch]
            for t_s in range(1, num_tokens):# 1,2,3,...,num_tokens-1, find the max prob token
    ...
    def _parse_config(cls, config, vocab, c_vocab):
        ...
        token_embs = TokenEmbedding(
            ...
            output_dim=config.get('embedding_reduce'))# reduce embed dim by linear projection
