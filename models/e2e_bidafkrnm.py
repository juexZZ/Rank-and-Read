...
class MyModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.ranker = knrm(num_bins, ranker_embedding_dim, ifcuda) # for the end2end+ model, CKNRM from encoded
    ...
    def _pack_and_unpack_lstm(cls, input, lengths, seq_encoder):
        ...
        #print(s_lengths.size(), indexes.size())
        #print(input.size()) 
    ...
    def _get_ranking_score(#self, enc_question, enc_passage, mask_q, mask_p, num_p_perQ):
        self, passage, p_lengths, question, q_lengths, num_p_perQ):
        '''
        ...
        # assert len(ranking_scores) == enc_passage.size()[0]
        ...
        for num in list(num_p_perQ.values()):
            ...
            log_norm_perQ = nn.functional.log_softmax(perQ, dim=0)
            #norm_perQ = nn.functional.softmax(perQ-torch.max(perQ),dim=0)
        ...
        #print(log_norm_ranking_scores)
        return log_norm_ranking_scores
    ...
    def forward(self, passage, p_lengths, question, q_lengths, num_p_perQ, p_labels):
        ...
        #ranking_scores = self._get_ranking_score(enc_question, enc_passage, q_mask, p_mask, num_p_perQ)
        ...
        #self._attention(selected_p_enc, selected_q_enc, p_mask, q_mask)
            #self._attention(enc_passage, enc_question, p_mask, q_mask)
        ...
        return log_start_probs, log_end_probs, ranking_scores

    @classmethod
    def get_loss(cls, log_start_probs, log_end_probs, ranking_scores, starts, ends, p_labels, num_p_perQ):
        """
        Get the loss, 
        """
        ...
        B,P = log_start_probs.size()
        assert B == len(starts)
        assert B == len(ends)
        
        #assert log_end_probs.requires_grad == True
        #assert log_start_probs.requires_grad == True

        def markTensor(labels, batch, p_lengths, end=False):
            '''
            expand starts and ends label to B x P tensor
            '''
            pos_tensor=torch.zeros(batch, p_lengths).cuda()
            for i in range(len(labels)):
                if end:
                    pos_tensor[i,labels[i].item()-1]= 1
                else:
                    pos_tensor[i,labels[i].item()]= 1
            pos_tensor=Variable(pos_tensor,requires_grad=False)
            return pos_tensor

        starts = markTensor(starts, B, P)# position(x,y): yth word in xth passage is the position
        ends = markTensor(ends, B, P, end=True)

        selected_start_probs, _ = torch.min(starts*log_start_probs,dim=1)
        selected_end_probs, _ = torch.min(ends*log_end_probs,dim=1)
        assert selected_end_probs.requires_grad == True
        assert selected_start_probs.requires_grad == True

        
        #if only train true
        ranking_scores = ranking_scores.index_select(0, p_labels.nonzero().squeeze())
        

        B_1,P_1 = starts.size()
        B_2 = len(ranking_scores)
        assert B == B_1
        assert P == P_1
        assert B_2 == B, (B,B_2)
        assert len(selected_start_probs) == B
        
        
        # if only train true
        loss = - torch.mean(selected_start_probs + selected_end_probs + ranking_scores)
        return loss
    ...
    def from_checkpoint(cls, config, checkpoint, ifcuda):
        ...
        model.load_state_dict({
            ...
            for name, val in
            checkpoint['model'].items() if name != 'ranker_embedder.embeddings.weight'})# embedding share
        model.eval()
