...
def save_vocab(checkpoint, path, id_to_symb):
    ...
    """
    vocab = np.array([
        id_to_symb[str(ind)] for ind in range(len(id_to_symb))])
...
def checkpoint(model, epoch, optimizer, dest, exp_folder,checkpointfile):
    """
    Checkpoint the current state of training.
    dest i.e. destination, the file `checkpoint` in main()
    """
    save_model(model, dest)
    save_epoch(epoch, dest)
    torch.save(optimizer.state_dict(),
               os.path.join(exp_folder, checkpointfile+'.opt'))
    return
